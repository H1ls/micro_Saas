...
| Hide unrelated panels in LLM and file error states | DONE | 2026-09-01 15:37:50 +03:00 | `not committed` | Fallback/LLM error dashboard no longer renders `InsightHero`; file upload errors no longer render the `UploadPanel`/`Анализ данных` window next to the Empty State. |
| Frontend background image | DONE | 2026-09-01 15:46:28 +03:00 | `not committed` | Added `back.png` as a Vite public asset and applied it as the app background with a light overlay to preserve glassmorphism readability. |
| Chart surface and Q&A dock polish | DONE | 2026-09-01 16:03:06 +03:00 | `not committed` | Chart panels now use near-opaque white surfaces, thinner ghost gridlines, mostly hidden Y axis, darker X labels, rounded bars, stable teal/slate/amber colors and white tooltips; AskPanel is visually separated as a darker dock without teal input focus border. |
| Raw text LLM-selected chart specs | DONE | 2026-09-01 16:18:10 +03:00 | `not committed` | Raw text extraction now asks LLM to return chart specs with type `bar`, `line` or `pie`; backend validates `x_key`, `y_key` and chart filters before `chart_service` prepares data for frontend. |
| Russian logical chart titles and readable chart labels | DONE | 2026-09-01 16:29:49 +03:00 | `not committed` | Analysis prompts now require logical Russian insight/chart titles when source data is Russian; chart X labels wrap into multiple lines inside the panel, and line charts no longer draw value labels over the chart. |
| CSV/Excel LLM chart filter resilience | DONE | 2026-09-01 16:57:54 +03:00 | `not committed` | Diagnosed `valid_sales` chart failure: LLM returned raw-text-only `group/metric_key` filters for CSV/Excel columns. Backend now drops invalid chart filters while preserving valid `x_key/y_key` chart specs, and prompts tell LLM to use filters only for real columns. |
| Mascot empty states for chart and generic errors | DONE | 2026-09-01 17:16:54 +03:00 | `not committed` | Added shared `MascotEmptyPanel` based on the LLM fallback mascot and used it for empty app state, fallback data state, generic analysis errors, empty `ChartGrid`, and unbuildable chart specs; `AskPanel` was left unchanged. |
| Tech debt cleanup P1/P2/P3 | DONE | 2026-09-01 18:05:09 +03:00 | `aa5c5e5` | Moved `/api/analyze` orchestration to `dashboard_service`, removed duplicate `AnalyzeResponse` assembly, split raw-text extraction facade, removed empty deterministic extraction stub and unused `DegradedStateNotice`, unified file-upload error state through `MascotEmptyPanel`, and stopped returning validation `details`. |
