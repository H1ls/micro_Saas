  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-1">
        <h2 className="text-lg font-semibold text-slate-950">Вопросы к данным</h2>
        <p className="text-sm leading-6 text-slate-600">Ответы ограничены загруженным dataset.</p>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {messages.length === 0 && (
          <div className="flex items-start gap-3 rounded-md border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-600">
            ...
            <p>Задайте вопрос, на который можно ответить по загруженным колонкам и строкам.</p>
          </div>
        )}
        {messages.map((message) => (
          <div key={message.id} className="rounded-md border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-700">{displayAnswer(message.response.answer)}</p>
            <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
              <span className="rounded-md bg-white px-2 py-1">Уверенность: {message.response.confidence}</span>
      ...
      {error && (
        <p className="mt-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm leading-6 text-red-700">
          {error} Загрузите dataset заново, если сессия истекла.
        </p>
      )}
